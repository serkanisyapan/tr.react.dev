...
Bir Efekt yazdığınızda, linter, Efektin bağımlılıkları listesine Efektin okuduğu her reaktif değeri (prop'lar ve state gibi) dahil ettiğinizi doğrulayacaktır. Böylelikle Efektiniz bileşeninizin son prop'ları ve state'i ile senkronize kalmış olur. Gereksiz bağımlılıklar Efektinizin çok sık çalışmasına neden olabilir ya da sonsuz bir döngü yaratabilir. Efektinizden gereksiz bağımlılıkları kaldırmak için bu rehberi takip edin.

</Intro>

<YouWillLearn>

- Sonsuz Efekt bağımlılığı döngüleri nasıl düzeltilir
- Bir bağımlılığı kaldırmak istediğinizde ne yapmalısınız
- Efektinizdeki bir değeri ona "tepki" vermeden nasıl okursunuz
- Nesne ve fonksiyon bağımlılıklarından nasıl ve neden kaçınırız
- Bağımlılık linter'ını görmezlikten gelmek neden tehlikelidir ve onun yerine ne yapılmalıdır

</YouWillLearn>

## Bağımlılıklar ile kod eşleşmelidir {/*dependencies-should-match-the-code*/}

Bir Efekt yazdığınızda, ilk önce Efektinizin yaptığı şeyi nasıl [başlatacağı ve durduracağını](/learn/lifecycle-of-reactive-effects#the-lifecycle-of-an-effect) belirtirsiniz:
...
Daha sonra, eğer Efekt bağımlılıklarını boş bırakırsanız (`[]`), linter doğru bağımlılıkları önerecektir:
...
Linter'ın dediklerine göre bağımlılıkları doldurun:
...
[Efektler reaktif değerlere "tepki" verirler.](/learn/lifecycle-of-reactive-effects#effects-react-to-reactive-values) `roomId` reaktif bir değer olduğundan (bir yeniden render sonucu değişebilir), linter, bu değeri bağımlılık olarak bildirdiğinizi doğrulayacaktır. Eğer `roomId` farklı bir değer alırsa React, Efektinizi yeniden senkronize edecektir. Bu, sohbetin seçili odaya bağlı kalmasını ve aşağı açılır menüye "tepki" vermesini sağlar:
...
### Bir bağımlılığı kaldırmak için onun bir bağımlılık olmadığını kanıtlayın {/*to-remove-a-dependency-prove-that-its-not-a-dependency*/}

Efektinizin bağımlılıklarını "seçemediğinize" dikkat edin. Efektinizin kodu tarafında kullanılan her <CodeStep step={2}>reaktif değer</CodeStep> bağımlılık listenizde bildirilmek zorundadır. Bağımlılık listesi Efektteki kod tarafından belirlenir:
...
function ChatRoom({ roomId }) { // Bu reaktif bir değerdir
  useEffect(() => {
    ...
    connection.connect();
    return () => connection.disconnect();
  }, [roomId]); // ✅ Yani bu reaktif değeri Efektinizin bir bağımlılığı olarak bildirmek zorundasınız
  // ...
}
```

[Reaktif değerler](/learn/lifecycle-of-reactive-effects#all-variables-declared-in-the-component-body-are-reactive), direkt olarak bileşeniniz içinde bildirilmiş olan prob'ları, değişkenleri ve fonksiyonları içerir. `roomId` reaktif bir değer olduğundan, bu değeri bağımlılık listesinden kaldırmazsınız. Linter buna izin vermeyecektir:
...
function ChatRoom({ roomId }) {
  useEffect(() => {
    ...
    connection.connect();
    return () => connection.disconnect();
  }, []); // 🔴 React Hook'u useEffect'in eksik bir bağımlılığı var: 'roomId'
  // ...
}
```

Ve linter doğru olacaktır! `roomId` değeri zaman içinde değişebileceğinden dolayı, bu durum kodunuzda bazı hatalara yol açacaktır.

**Bir bağımlılığı kaldırmak için, linter'a o bağımlılığa *ihtiyaç olmadığını* "kanıtlamanız" gerekmektedir.** Örneğin, `roomId` değişkenini bileşenin dışına taşıyarak bu değişkenin reaktif olmadığını ve yeniden render'lar ile değişmeyeceğini kanıtlayabilirsiniz:

```js {2,9}
const serverUrl = 'https://localhost:1234';
const roomId = 'music'; // Artık reaktif bir değer değildir
